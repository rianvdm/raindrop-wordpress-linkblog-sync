...
describe('WordPressClient', () => {
  ...
  describe('createPost', () => {
    ...
    it('should generate correct Basic Auth header', async () => {
      ...
      const authHeader = (mockFetch.mock.calls[0][1]?.headers as any)?.['Authorization'];
    ...
    it('should handle WordPress API errors', async () => {
      ...
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow(WordPressError);
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow('Invalid parameter(s): title');
    });
    ...
    it('should handle authentication errors', async () => {
      ...
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow(WordPressError);
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow('not allowed to create posts');
    });
    ...
    it('should handle non-JSON error responses', async () => {
      ...
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow('Server error occurred');
    });
    ...
    it('should validate response has required fields', async () => {
      ...
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow('missing required fields');
    });
    ...
    it('should handle network errors', async () => {
      ...
      await expect(client.createPost(mockCreatePostPayload)).rejects.toThrow('Failed to create WordPress post: Network failure');
    });
