...
describe("KVStorageService", () => {
  ...
  describe("getLastFetchTime", () => {
    ...
    it("should return the stored last fetch time", async () => {
      ...
      await mockKV.put(
        KV_KEYS.LAST_FETCH,
        JSON.stringify({ timestamp: testDate.toISOString() })
      );
  ...
  describe("markItemAsPosted", () => {
    ...
    it("should allow re-marking items as posted", async () => {
      ...
      await expect(
        service.markItemAsPosted(raindropId)
  ...
  describe("error handling", () => {
    ...
    it("should throw error when setLastFetchTime fails", async () => {
      ...
      await expect(service.setLastFetchTime(new Date())).rejects.toThrow(
    ...
    it("should throw error when markItemAsPosted fails", async () => {
      ...
      await expect(service.markItemAsPosted("test-id")).rejects.toThrow(
        "Failed to mark item test-id as posted"
      );
    });
  });
});
