...
describe('SyncOrchestrator', () => {
  ...
  describe('performSync', () => {
    it('should perform complete sync successfully', async () => {
      ...
      expect(mockRaindropClient.fetchBookmarks).toHaveBeenCalledWith('blog', lastFetchTime);
    ...
    it('should handle Raindrop fetch errors', async () => {
      ...
      expect(mockLogger.logError).toHaveBeenCalledWith(fetchError, expect.objectContaining({
    ...
    it('should handle WordPress post creation errors', async () => {
      ...
      expect(result.errors).toContain('Failed to process bookmark new-bookmark-1: WordPress API error');
    ...
    it('should handle KV storage errors gracefully', async () => {
      ...
      expect(mockLogger.logWarning).toHaveBeenCalledWith(
        expect.stringContaining('Failed to check if item new-bookmark-1 was posted'),
        expect.objectContaining({ bookmarkId: 'new-bookmark-1' })
      );
    });

    it('should not update timestamp if there are errors', async () => {
      const bookmark1 = { ...mockRaindropItem, _id: 'good-bookmark', title: 'Good Bookmark' };
      const bookmark2 = { ...mockRaindropItem, _id: 'bad-bookmark', title: 'Bad Bookmark' };
